                    ...
                    <form method="POST" action="{{ route('projects.update', $project) }}">
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('cell_id') is-invalid @enderror" id="cell_id" name="cell_id" required>
                                ...
                                @foreach($cells as $cell)
                                    <option value="{{ $cell->id }}" {{ old('cell_id', $project->cell_id) == $cell->id ? 'selected' : '' }}>
                                        {{ $cell->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('manager_id') is-invalid @enderror" id="manager_id" name="manager_id" required>
                                ...
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ old('manager_id', $manager ? $manager->id : '') == $user->id ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-3">
                            ...
                            <select class="form-select @error('team_members') is-invalid @enderror" id="team_members" name="team_members[]" multiple>
                                @foreach($users as $user)
                                    <option value="{{ $user->id }}" {{ (is_array(old('team_members', $teamMembers)) && in_array($user->id, old('team_members', $teamMembers))) ? 'selected' : '' }}>
                                        {{ $user->name }}
                                    </option>
                                @endforeach
                            </select>
...
<script>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        // You can use a library like Choices.js or Select2 here
    });
</script>
@endsection